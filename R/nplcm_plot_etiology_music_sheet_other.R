...
nplcm_plot_etiology_music_sheet_other <- function(DIR_list,
  ...
  for (i in seq_along(DIR_list)){
    ...
    for (j in 1:d2_nplcm){
      SubVarName = paste(VarName,"[",j,"]",sep="")
      Icat_nplcm[,j] = res_nplcm[,SubVarName]
    }
    ...
    row.names(res)=sapply(x.levels,function(x) pathogen_list[[i]][x])
    ...
    if (ncol(res.ggplots)<length(pathogen_list[[i]])){
    ...
    colnames(res.ggplots)=pathogen_list[[i]]
    res.ggplots[is.na(res.ggplots)]=0
    res.ggplots[res.ggplots<0.03]=0
    res.ggplots$NoA = 1-rowSums(res.ggplots)
    pEti_raw=res.ggplots    
    ...
    pathogen_names[[i]] = pathogen_names[[i]]
